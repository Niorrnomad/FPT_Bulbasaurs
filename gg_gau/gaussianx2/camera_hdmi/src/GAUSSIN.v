module gaussian_filter #(
    ...
    wire [5:0] gray_in=i_data[10:5]; // viết lại cái code C cho MCU thì đổi đường data đi hì
    ...
    reg [5:0] d1, d2, d0_reg; // xóa cái d3 đi giùm t cái
    wire [5:0] d0 = gray_in;

    always @(posedge clk) begin
        if (i_href) begin
        d1<=line_buff_0[wr_ptr];
            d2<=line_buff_1[wr_ptr]; // đi lại line buffer đoạn này 
            d0_reg<=d0;
...
always @(posedge clk or negedge rst_n) begin
    ...
    always @(posedge clk or negedge rst_n) begin
        ...
        final_sum <= h0 + (h1 << 1) + h2; // shift này là chia 2 đó cha
        valid_out <= 1;
        end else begin
            valid_out <= 0;
        end
